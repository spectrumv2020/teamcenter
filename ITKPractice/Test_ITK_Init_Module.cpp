#include "Test_ITK_Init_Module.h"
...
static int report_error(char* file, int line, char* call, int status,
    logical exit_on_error) { //report_error
    if (status != ITK_ok)  {
        int
            n_errors = 0,
            * severities = NULL,
        ...
        log("error %d at line %d in %s\n", status, line, file);
        log("%s\n", call);

        if (exit_on_error) {
            log("%s", "Exiting program!\n");
            exit(status);
        }
    }
...
int ITK_user_main(int argc, char* argv[]){ //ITK_user_main
    ...
    log("Calling ITK_init_module(%s, %s, %s)...\n", users_id, users_password,
        users_group);
