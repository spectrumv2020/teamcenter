...
public class GridItem extends Item {
	...
	public void setExpanded(boolean expanded) {
		...
		if (hasChildren)
			...
			{
				/*********Added By RP to use this in filter*****/
				Object myKey = item.getData("KEY_FILTER_STATUS");
				if (myKey != null)
				{
					if((myKey.toString().equals("hidden")) || (myKey.toString().equals("N")))
					{
					...
					item.setVisible(expanded && visible);
				for (GridItem childitem : item.getItems())
				{
					Object myKey1 = childitem.getData("KEY_FILTER_STATUS");
					if (myKey1 != null)
					{
						try{
						if((myKey1.toString().equals("hidden")) || (myKey1.toString().equals("N")))
						{
					...
					/*else
						childitem.setVisible(expanded && visible);*/
				}
				/*********End of Logic*****/
	 ...
	 *            The visible to set.
	 */
	public void e2_setVisible(boolean visible) 
	{
