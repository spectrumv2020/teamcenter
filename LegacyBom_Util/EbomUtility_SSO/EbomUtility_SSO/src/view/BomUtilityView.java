...
public class BomUtilityView {
	...
	public static String sTC_SSO_URL = "";
	public static String sTC_SSO_APP_ID = "";
	public static String sTC_SESSION_DISCRIM = "EBOMUtility_";
	...
	public static void print_help() {
		String strHelp = "%JAVA_HOME%\\bin\\java -cp %TCSSO_SESSION_AGENT_PATH%/lib\\*;EBOM_Utility.jar %IP_CONFIG_PROP% " +
				"\nWhere, TCSSO_SESSION_AGENT_PATH -- Teamcenter Security Agent Path" +
				"\n IP_CONFIG_PROP -- Teamcenter Security Agent Path";
	...
	public static void main(String[] args) {
		...
		if (args.length == 1)
		{
			...
			writeLogFile(sInputLogFilePath, "Please run the Program with neccessary input paramater");
			print_help();
			return;
		}
		try {
			sTC_SESSION_DISCRIM += smpd.format(new Date()).toString().replaceAll(":", "_");
	...
	protected void createContents() throws Exception{
		...
		try {
			...
			sTC_SSO_URL = prop.getProperty("TC_SSO_URL");
			sTC_SSO_APP_ID = prop.getProperty("TC_SSO_APP_ID");
		...
		hmKeyMappingVsAvailability.put("TC_SSO_URL", false);
		hmKeyMappingVsAvailability.put("TC_SSO_APP_ID", false);
			...
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					if(user == null) {
						...
						try {
							user = session.ssoLogin(sTC_SSO_URL, sTC_SSO_APP_ID);
						} catch(Exception ex) {
							printTCURLInLog(sDryRunLogFilePath);
							writeLogFile(sDryRunLogFilePath, "Login Failed...!");
							getMessageBox(shlBomUtility, SWT.ERROR, "ERROR", "Teamcenter Login Failed..!"); 
						...
						for (int i = 0; i < gdItems.length; i++) {
							...
							if(i==0) {
								...
								if(eBomObj!=null && eBomObj.length>0){
									...
									/*try {
										String sObjType = ((Item)eBomObj[0]).get_object_type();
										if(!sObjType.equalsIgnoreCase(prop.getProperty("FIRST_LINE_ITEM_TYPE"))) {
											sPWBErrors = sPWBErrors + ("ERROR Excel Row Number : " + (i + 2)
													+ " - PWB Item (" + gdItems[i].getText(iColPartNumber)
													+ ") is available with different object type\n");
										}
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					...
					try {
						...
						printTCURLInLog(sDryRunLogFilePath);
						writeLogFile(sExecuteLogFilePath, "Login Failed...!");
						getMessageBox(shlBomUtility, SWT.ERROR, "ERROR", "Teamcenter Login Failed..!"); 
						return;
					}
	...
	public void printTCURLInLog(String sLogFilePath) {
		try {
			sTcUserName = user.get_user_id();
		}catch(Exception ex){
			sTcUserName = "";
		}
		writeLogFile(sLogFilePath, "Logging in to TC_Server : "+serverHost+
				"\n\t TC_SSO_Url : " + sTC_SSO_URL+
				"\n\t TC_APP_ID : " + sTC_SSO_APP_ID+
				"\n\t TC_User : "+sTcUserName);
