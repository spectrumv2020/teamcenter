extern DLLAPI int item_save_post_action(METHOD_message_t *m, va_list args)
{
    /***  va_list for TC_save_msg  ***/
    tag_t item  = va_arg(args, tag_t);
    /***********************************/

    printf( "\t item_save_post_action - item: %u\n", item);
...
extern int register_item_save_post_action()
{
    int 
        ec = ITK_ok; 
    METHOD_id_t 
    ...
    {    
        ERROR_CHECK( METHOD_add_action(method, METHOD_post_action_type,
            (METHOD_function_t) item_save_post_action, user_args));
        printf( " - successful!");
    }
    else
    {
