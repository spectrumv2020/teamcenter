/*HEAD REGISTER_POST_ACTION_WITH_ARGUMENTS CCC ITK */
extern DLLAPI int register_post_action(char *type, char *operation, char *extension_point, METHOD_function_t function_ptr)
{
    ...
    if (method.id != NULLTAG)
    {
        ...
        IFERR_REPORT( METHOD_add_action(method, METHOD_post_action_type, function_ptr, user_args));
