/*HEAD REGISTER_PRE_CONDITION_WITH_ARGUMENTS CCC ITK */
extern DLLAPI int register_pre_condition(char *type, char *operation, char *extension_point, METHOD_function_t function_ptr)
{
    ...
    if (method.id != NULLTAG)
    {
        ...
        IFERR_REPORT( METHOD_add_pre_condition(method, function_ptr, user_args));
