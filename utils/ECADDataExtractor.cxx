...
Logger *logger = Logger::getLogger("ECADDataExtractor");
...
int main(int argc, char **argv) {
    ...
    //printItemData(items);
    writeToFile(items, objectLocations);
...
void writeToFile(map<string, map<tag_t, TCItemData_t> > items, map<tag_t,set<string> > objectLocations ){
    map<string, map<tag_t, TCItemData_t> > ::iterator itr;
    ...
    fprintf(fpMetaData, "Current ID||Current Name||Current Revision||Current Description||Creation Date||Date Modified||Owner||Last Modifying User||Release Status||Type||Part Location||Part Rev Location||Document Location");        
    ...
    for(itr = items.begin(); itr != items.end(); ++itr){
        ...
        for(itr1 = itemMap.begin(); itr1 != itemMap.end(); ++itr1){
            TCItemData_t itemData = itr1->second;
            string itemLocations = getObjectLocations(itemData.itemTag, objectLocations);
            map<tag_t, TCRevData_t> revMap = itemData.revData;
            map<tag_t, TCRevData_t>::iterator itr2;
            for(itr2 = revMap.begin(); itr2 != revMap.end(); ++itr2){
                TCRevData_t revData = itr2->second;
                string revLocations = getObjectLocations(revData.revTag, objectLocations);
                map<tag_t,TCDatasetData_t> datasetMap = revData.datasetData;
                ...
                for (itr3 = datasetMap.begin(); itr3 != datasetMap.end(); ++itr3 ){
                    ...
                    if (fileMap.size() == 0){
                        ...
                        fprintf(fpMetaData, "%s||%s||%s||%s||%s||%s||%s||%s||%s||%s||%s||%s||%s", 
                                        itemData.itemId.c_str(),
                                        datasetData.datasetName.c_str(),
                                        revData.revId.c_str(),
